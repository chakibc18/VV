...
public class Class_X_RepresentationPrinter {
	 
	static void print_x (NODE entry) {
		  if (entry ==null) return ;

		print_switch(entry);
		   for (NODE x : entry.children)
			   Class_X_RepresentationPrinter.print_x(x) ;

	}
	
	
	  static void print_switch(NODE nX) {
		  if(nX instanceof X_CtPackageImpl) { 
			  X_CtPackageImpl n=(X_CtPackageImpl) nX;

System.out.println(n.node_representation);
return ;
}
		  if(nX instanceof X_CtClassImpl) {
			  X_CtClassImpl n=(X_CtClassImpl) nX;
			  System.out.println(ClassUtil.linearize_code_lines(n));
