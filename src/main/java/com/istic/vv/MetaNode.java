...
public class MetaNode {
	...
	public MetaNode(MetaNode parent) {
		this.LQNodes= new ArrayList<NODE>();
		this.WQNodes= new ArrayList<NODE>();
//		this.WQNodes.add( new X_NONE());
//		this.WQNodes.add( new X_NONE());
		this.parent = parent;
		this.NQMetaNodes= new ArrayList<MetaNode>();
		counter++;
...
return MetaNode.nb_changed;
	...
	public static String print(MetaNode root ) {
		MetaNode.toPrint="";
		MetaNode.print_x(root, "");
		return MetaNode.toPrint;
	}
	private static void print_x(MetaNode entry, String tmp) {
String res= tmp;
if (tmp.compareTo("")!=0) res= entry.toString();
for (MetaNode n : entry.NQMetaNodes) {
	res+="\n"+n.toString();
}
for (MetaNode n : entry.NQMetaNodes) {
MetaNode.print_x(n,res);
}
//return "[[[\n"+res+"]]]\n";
	}
